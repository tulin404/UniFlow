...
export default function LessonElement({ actCourse, actName, actLink, dueDate, done, status, theme }) {
    const BadgeIcon = () => {
        if (done) return <div className="flex justify-center items-center"><CheckCircleIcon className={`${theme === "dark" ? "text-primary" : "text-green-500"} size-6 md:size-6.5 stroke-[1.5]`} /></div>
    ...
    return (
        <article className={`${done ? "opacity-70 bg-card-done border-card-done-border hover:bg-card-done-hover" : "opacity-100 bg-card hover:bg-card-hover border-card-border"} font-[Inter] transition-all duration-200 rounded-md border-2 h-full px-4 py-2`}>
            ...
            {done && <div className="flex pl-8.5 md:pl-9 items-center"><h4 className="act-course text-sm md:text-base text-text-soft font-medium">{actCourse}</h4></div>}
        </article>
    );
};