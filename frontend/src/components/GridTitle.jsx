    ...
    return (
        <div className="title flex flex-col justify-center items-start text-text">
            <button onClick={() => setActive(!active)} className="flex group peer gap-1 sm:gap-3 items-center justify-center font-semibold font-[Inter]">
                ...
                <ArrowRightIcon className={`${active ? "rotate-90 opacity-100" : "rotate-0 sm:opacity-0"} h-7.5 xl:h-8.5 stroke-[2.5] font-extrabold opacity-0 group-hover:flex group-hover:opacity-100 hover:bg-hover-bg active:scale-95 active:bg-active-bg rounded-full aspect-square p-1 transition-all duration-200`} />
                <h2 className={`${active ? "sm:translate-x-0 xl:translate-x-0" : "-translate-x-4 sm:-translate-x-7"} text-4xl group-hover:scale-110 xl:group-hover:scale-100 xl:text-5xl xl:-translate-x-9 group-hover:translate-x-0 font-semibold active:scale-90 xl:active:scale-95 font-[satoshi] transition-all duration-200 z-3`}>{name}</h2>
            </button>
