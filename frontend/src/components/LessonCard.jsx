...
export default function LessonCard({ actCourse, actName, actLink, dueDate, done, state, lastMod, theme, childrenNum }) {
    ...
    return (
        <a href={actLink} target="_blank" rel="noopener noreferrer" className="h-full">
            <article className={`opacity-100 ${styles.bg} ${styles.hover} ${styles.border} flex flex-col font-[Inter] transition-all duration-200 rounded-md border-2 hover:-translate-y-1 px-4 py-2 gap-2 h-full ${childrenNum <= 1 ? "w-[50dvw] sm:w-[45dvw] md:w-[40dvw] lg:w-[35dvw] xl:w-[25dvw]" : ""}`}>
